export function TeamHero() {
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-background to-secondary-50">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-5xl sm:text-6xl font-bold mb-6 text-balance leading-tight text-foreground">
          Team, Advisors & Investors
        </h1>
        <p className="text-xl text-foreground/80 max-w-2xl">
          Meet the team, advisors, and investors behind AGI Ventures Canada. We're dedicated to supporting Canadian AI and robotics founders on their journey from idea to impact.
        </p>
      </div>
    </section>
  )
}

